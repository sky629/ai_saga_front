...
function extractHpChange(data: GameActionResponse): number {
    ...
    return data.message.parsed_response?.state_changes?.hp_change ?? 0;
}
...
function extractProgressionStateChanges(
    ...
    return data.message.parsed_response?.state_changes ?? null;
}
...
function buildProgressionStatusFromSession(
    ...
    return {
        ...
        escape_status:
            typeof sessionData.game_state.escape_status === 'string'
                ? sessionData.game_state.escape_status
                : undefined,
    };
}

function buildProgressionStatusFromAchievementBoard(
    achievementBoard: ProgressionAchievementBoard,
    totalTurns: number
): ProgressionStatusPanel {
    return {
        hp: achievementBoard.hp,
        max_hp: achievementBoard.max_hp,
        internal_power: achievementBoard.internal_power,
        external_power: achievementBoard.external_power,
        manuals: achievementBoard.manuals,
        remaining_turns: achievementBoard.remaining_turns,
        elapsed_turns: totalTurns,
        escape_status: achievementBoard.escaped
            ? '동굴을 돌파할 자격을 증명했습니다.'
...
export default function GameSession() {
    ...
    const [shouldAnimateInitialSystemMessage, setShouldAnimateInitialSystemMessage] =
        useState(false);
    ...
    const character = characters?.find(
        c => c.id === sessionData?.character_id
    );
    ...
    useEffect(() => {
        const finalOutcome = sessionData?.final_outcome ?? null;
        setAchievementBoard(finalOutcome?.achievement_board ?? null);
        if (sessionData?.game_state) {
            if (finalOutcome?.achievement_board) {
                setProgressionStatus(
                    buildProgressionStatusFromAchievementBoard(
                        finalOutcome.achievement_board,
                        sessionData.turn_count
                    )
                );
                return;
            }
            setProgressionStatus((current) => {
                return buildProgressionStatusFromSession(sessionData) || current;
            });
        }
    }, [sessionData]);
    ...
    const currentScenario =
        ...
        null;
    const currentGameType = currentScenario?.game_type;
    const statusPanelHp =
    ...
    const sendActionMutation = useMutation({
        ...
        onSuccess: (data) => {
            ...
            if (!isGameActionResponse(data)) {
                ...
                if (finalOutcome.achievement_board) {
                    setAchievementBoard(finalOutcome.achievement_board);
                    setProgressionStatus(
                        buildProgressionStatusFromAchievementBoard(
                            finalOutcome.achievement_board,
                            data.total_turns
                ...
                const endingMessage: GameMessageResponse = {
                    id: `ending-${Date.now()}`,
                    role: 'system',
                    content: data.final_outcome.narrative,
                    parsed_response: {
                        narrative: data.final_outcome.narrative,
                        options: [],
                        ending_type: data.ending_type,
                        final_outcome: data.final_outcome,
                    },
                    image_url: data.final_outcome.image_url ?? null,
                    ...
                    >
                        <div className="flex flex-col h-full min-h-0 relative">
                            <div className="bg-sanabi-panel text-sanabi-cyan px-4 py-2 font-bold text-center border-b border-sanabi-cyan/30 shrink-0 z-20 flex justify-between items-center shadow-[0_4px_20px_rgba(0,240,255,0.1)]">
                                <span className="text-xs uppercase tracking-widest text-sanabi-cyan/70">
                                    System.Log
                                </span>
                                <span className="tracking-widest animate-pulse">
                                    {`// ${
                                        currentScenario?.name ||
                                        sessionData.scenario_id.substring(0, 12)
                                    }`}
                                </span>
                        ...
                        <div className="flex flex-col h-full min-h-0">
                            ...
                            <div className="flex-1 overflow-y-auto min-h-0">
                                <GameStatePanel
                                    ...
                                    achievementImageUrl={sessionData.image_url}
                                />
                            </div>
                        </div>
                    </PixelCard>
